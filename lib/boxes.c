...
void free_boxes(Box *boxes)
{
	...
	for(ibox=0;ibox<n_boxes;ibox++) {
		if(boxes[ibox].n_obj>0) {
			free(boxes[ibox].pos);
			free(boxes[ibox].weight);
			free(boxes[ibox].index);
			free(boxes[ibox].visit_min);
			free(boxes[ibox].visit_max);
		}
	}
...
{
	size_t idim,iobj;
	size_t n_tot1=cat1.n_obj*dim_box;
	size_t n_tot2=cat2.n_obj*dim_box;
	n_obj_tot = cat1.n_obj+cat2.n_obj;
	...
	for (idim=0;idim<dim_box;idim++) {
		bound_min[idim]=cat1.box[idim];
		bound_max[idim]=cat1.box[idim];
	}
	
	for(iobj=0;iobj<n_tot1;iobj+=dim_box) {
		for (idim=0;idim<dim_box;idim++) {
			size_t kk=iobj+idim;
			bound_min[idim] = MIN(bound_min[idim],cat1.box[kk]);
			bound_max[idim] = MAX(bound_max[idim],cat1.box[kk]);
		}
	}
	
	for(iobj=0;iobj<n_tot2;iobj+=dim_box) {
		for (idim=0;idim<dim_box;idim++) {
			size_t kk=iobj+idim;
			bound_min[idim] = MIN(bound_min[idim],cat2.box[kk]);
			bound_max[idim] = MAX(bound_max[idim],cat2.box[kk]);
		}
	}
...
{
	size_t iobj,ibox,idim,nfull;
	size_t *index=(size_t *) malloc(dim_box*sizeof(size_t));
	histo_t *pos=(histo_t *) malloc(dim_pos*sizeof(histo_t));
	Box *boxes;

	boxes=init_boxes(n_boxes);
	
	nfull=0;
	for(iobj=0;iobj<cat.n_obj*dim_box;iobj+=dim_box) {
		box2ind(&(cat.box[iobj]),index);
		size_t ibox=ravel_index(index,n_side,dim_box);
		if(boxes[ibox].n_obj==0) nfull++;
		boxes[ibox].n_obj++;
	}
	*n_box_full=nfull;
#ifdef _VERBOSE
	printf(" - There are %zu objects in %zu out of %zu boxes \n",cat.n_obj,nfull,n_boxes);
#endif //_VERBOSE
	*box_indices=(size_t *)malloc(nfull*sizeof(size_t));
	if(box_indices==NULL) error_mem_out();
	
	nfull=0;
	for(ibox=0;ibox<n_boxes;ibox++) {
		if(boxes[ibox].n_obj>0) {
			boxes[ibox].pos=(histo_t *)malloc(dim_pos*boxes[ibox].n_obj*sizeof(histo_t));
			if(boxes[ibox].pos==NULL) error_mem_out();
			boxes[ibox].weight=(histo_t *)malloc(dim_weight*boxes[ibox].n_obj*sizeof(histo_t));
	...
	for(iobj=0;iobj<cat.n_obj;iobj++) {
		...
		for (idim=0;idim<dim_pos;idim++) {
			boxes[ibox].pos[dim_pos*n_obj+idim]=pos[idim];
		}
		histo_t *weight=&(cat.weight[dim_weight*iobj]);
		for (idim=0;idim<dim_weight;idim++) {
			boxes[ibox].weight[dim_weight*n_obj+idim]=weight[idim];
		}
		boxes[ibox].n_obj++;
	}
