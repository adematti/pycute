...
void print_multi_type()
{
	...
	if (multi_type==MULTI_ALL) {
		...
		printf(" - ells:");
		for (ill=0;ill<n_ells;ill++) printf(" %zu",ill);
		printf("\n");
	}
	if (multi_type==MULTI_EVEN) {
		printf(" - multi-type: even\n");
		printf(" - ells:");
		for (ill=0;ill<n_ells;ill++) printf(" %zu",2*ill);
		printf("\n");
	}
	if (multi_type==MULTI_ODD) {
		printf(" - multi-type: odd\n");
		printf(" - ells:");
		for (ill=0;ill<n_ells;ill++) printf(" %zu",2*ill+1);
		printf("\n");
	}
}

void set_multi_type(char* type,size_t n_ells_)
{
...
void run_2pcf_multi(histo_t *meanmain,histo_t *count,size_t n_ells,char* multi_type,char *los_type,size_t n_threads)
{
	timer(0);
	set_num_catalogs();
#ifdef _VERBOSE
	...
	print_catalogs();
#endif //_VERBOSE
	set_multi_type(multi_type,n_ells);
	set_los_type(los_type);
	set_num_threads(n_threads);
	set_meshs(cats,meshs,n_cats);
#ifdef _VERBOSE
	timer(1);
#endif //_VERBOSE
	if (n_cats==1) auto_2pcf_multi(meshs[0],meanmain,count);
	else cross_2pcf_multi(meshs[0],meshs[1],meanmain,count);	
#ifdef _DEBUG
...
void run_3pcf_multi(histo_t *count,size_t n_ells,char* multi_type,char *los_type,size_t n_threads)
{
	timer(0);
	set_num_catalogs();
#ifdef _VERBOSE
	...
	timer(1);
#endif //_VERBOSE
	cross_3pcf_multi(meshs,n_cats,count);
#ifdef _VERBOSE
	printf("*** Cleaning up\n");
	timer(1);
	printf("\n");
...
void run_3pcf_multi_double_los(histo_t *count,size_t n_ells,char* multi_type,char *los_type,size_t n_threads)
{
	timer(0);
	set_num_catalogs();
#ifdef _VERBOSE
	...
	timer(1);
#endif //_VERBOSE
	cross_3pcf_multi_double_los(meshs,n_cats,count);
#ifdef _VERBOSE
	printf("*** Cleaning up\n");
	timer(1);
	printf("\n");
...
void run_2pcf_multi_radial(histo_t *count,size_t n_ells,char* multi_type,_Bool normalize,char *los_type,size_t n_threads)
{
	timer(0);
	set_num_catalogs();
#ifdef _VERBOSE
	...
	timer(1);
#endif //_VERBOSE
	cross_2pcf_multi_radial(meshs[0],meshs[1],count,normalize);
#ifdef _VERBOSE
	printf("*** Cleaning up\n");
	timer(1);
	printf("\n");
...
void run_4pcf_multi_radial(histo_t *count,size_t n_ells,char* multi_type,_Bool normalize,char *los_type,size_t n_threads)
{
	timer(0);
	set_num_catalogs();
#ifdef _VERBOSE
	...
	timer(1);
#endif //_VERBOSE
	cross_4pcf_multi_radial(meshs,count,normalize);
#ifdef _VERBOSE
	printf("*** Cleaning up\n");
	timer(1);
	printf("\n");
