...
def reference_smu(sedges,muedges,position1,weight1,position2=None,weight2=None,los='midpoint'):
...
def reference_multi(sedges,position1,weight1,position2=None,weight2=None,ells=[0,1,2,3,4],los='midpoint'):
	"""Reference pair counting via kdcount"""
	tree1 = correlate.points(position1,boxsize=None,weights=weight1)
	if position2 is None: tree2 = tree1
	else: tree2 = correlate.points(position2,boxsize=None,weights=weight2)
...
def test_smu():
	...
	countsref = reference_smu(sedges,muedges,position1,weight1,position2=position1,weight2=weight1,los='x')
	counts = pycute.counts + pycute.counts[:,::-1]
	testing.assert_allclose(countsref,counts,rtol=1e-7,atol=1e-7)
...
def test_multi():
	...
	for ells in [[0,1,2,3,4,5,6],[0,2,4,6,8],[1,3,5,7,9]]:
		...
		countsref = reference_multi(sedges,position1,weight1,ells=ells,los='x')
		counts = (1 + (-1)**scipy.asarray(pycute.ells))*pycute.counts
		testing.assert_allclose(countsref,counts,rtol=1e-7,atol=1e-7)
...
def test_los():
	...
	for ells in [[0,1,2,3,4,5,6],[0,2,4,6,8],[1,3,5,7,9]]:
		...
		pycute.set_2pcf_multi(sedges,position1,weight1,position2=position2,weight2=weight2,ells=ells,nthreads=nthreads,muedges=[-1.,1.],los='midpoint')
		middle = scipy.array(pycute.counts)
		pycute.set_2pcf_multi(sedges,position1,weight1,position2=position2,weight2=weight2,ells=ells,nthreads=nthreads,muedges=[-1.,1.],los='endpoint')
		print scipy.absolute(pycute.counts/middle-1.)[middle>0.].max()
	
	ells = [1,3,5,7,9]
	pycute.set_2pcf_multi(sedges,position1,weight1,position2=position1,weight2=weight1,ells=ells,nthreads=nthreads,muedges=[-1.,1.],los='midpoint')
	print pycute.counts

def save_reference_2pcf_multi_radial_leg():
	position1,weight1,position2,weight2 = load_catalogues()
	pycute = PyCute()
	pycute.set_2pcf_multi_radial_leg(sedges,position1,weight1,position2,weight2,ells=ells,nthreads=nthreads)
	scipy.save('ref_2pcf_multi_radial_leg.npy',pycute.counts)
		
def reference_2pcf_multi_radial_leg():
	return scipy.load('ref_2pcf_multi_radial_leg.npy')

def test_2pcf_multi_radial_leg():
	position1,weight1,position2,weight2 = load_catalogues()
	pycute = PyCute()
	pycute.set_2pcf_multi_radial_leg(sedges,position1,weight1,position2,weight2,ells=ells,nthreads=nthreads)
	countsref = reference_2pcf_multi_radial_leg()
	testing.assert_allclose(countsref,pycute.counts,rtol=1e-7,atol=1e-7)
...
save_reference_2pcf_multi_radial_leg()
"""
